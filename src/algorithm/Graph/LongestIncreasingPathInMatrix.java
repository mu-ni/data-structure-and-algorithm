...
public class LongestIncreasingPathInMatrix {
    ...
    public int longestIncreasingPath2(int[][] matrix) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                int len = dfs(matrix, i, j, Integer.MIN_VALUE, cache);
                max = Math.max(max, len);
            }
        }
        return max;
    ...
    public int dfs(int[][] matrix, int x, int y, int val, int[][] cache) {
        ...
        int max = 0;
        for (int[] dir : dirs) {
            max = Math.max(max, 1 + dfs(matrix, x + dir[0], y + dir[1], matrix[x][y], cache));
        }
        cache[x][y] = max;
        return max;
    }
